...
def readabs(frame):
    ...
    frame.information.delete("1.0",tk.END)
    wb = load_workbook(filename='./E_services/Abs.xlsx')
    ws = wb.get_sheet_by_name('Sheet1')
...
def readnotes(frame):
    ...
    frame.information.delete("1.0",tk.END)
    wb = load_workbook(filename='./E_services/Notes.xlsx')
    ws = wb.get_sheet_by_name('Sheet1')
        ...
        if(choice == 1):
            ...
            try:
                rescours = AnalyseCours.analyse(
                    inners=cours_download.connectEsv(writeFile=False, usr=usr, pwd=pwd, nbWeeks=10))
            except:
                rescours = 'bug'
            self.information.delete("1.0", tk.END)
            ...
            if (rescours == 'unchanged'):
                ...
                self.information.insert(tk.INSERT, 'Cours bugged\n')
            elif (rescours == 'calendar bug'):
                self.information.insert(tk.INSERT, 'New cours have been saved or some cours are changed, but pushing to calendar bugged\n')
